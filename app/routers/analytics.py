...
class TranslateSkillRequest(BaseModel):
    duty: str
    target_role: str | None = "AI Operations Specialist"
    country: str | None = "United States"


@router.post("/translate-skill")
def translate_skill_endpoint(req: TranslateSkillRequest) -> dict[str, Any]:
    """Translate traditional duty into AI-era human moat capability statement using Gemini AI."""
    from app.services.gemini import translate_legacy_duty
    return translate_legacy_duty(
        duty=req.duty,
        target_role=req.target_role or "AI Operations Specialist",
